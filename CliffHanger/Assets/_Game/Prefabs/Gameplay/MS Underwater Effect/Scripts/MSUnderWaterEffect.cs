...
public class MSUnderWaterEffect : MonoBehaviour
{
    ...
    [Header("Waters")]
    [Tooltip("Here it is possible to choose how the water will be detected, by collision (OnTriggerEnter), RayCast Hit or height of the water surface.")]
    ...
    [Header("Sounds")]
    [Range(0, 1)]
    [Tooltip("In this variable, it is possible to define the volume that the sounds reproduced by this code will have.")]
        ...
        if (waters != null)
        {
            if (waters.Length > 0)
            {
                ...
                for (int x = 0; x < waters.Length; x++)
                {
                    ...
                    { //If the string is null, it means that the array has just been created.
